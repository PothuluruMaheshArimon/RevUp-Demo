let osclerHost = "api-uat.digilytics.solutions";
//let osclerHost = "api-uat.digilytics.solutions";  //for working Locally
        ...
        menuItemDefinitions: {
            createComment: {
                onclick: function (event) {
					...
					command.reportDisplayName = this.renderTo.attributes[1].nodeValue;;
					// command.id = reportId;
					// command.pageName = "Geo-Hierarchy";
					// command.pageDisplayName ="Geo-Hierarchy"
                    command.filterData=filterData
					saveComment("report",command,this.renderTo.attributes[1].nodeValue);
			...
			shareData: {
                onclick: function () {
                  ...
                  let shareReportId  = dashboardId;
					...
					shareData("report",command,this.renderTo.attributes[1].value,this.getCSV())
...
var commentDataApiObject={};
  ...
  function submitComment(commentDataApiObject,text){
        ...
        if (text!=''){
            ...
            myObject.pageName = $('.reportTitle').text();
            myObject.pageDisplayName = $('.reportTitle').text();
            ...
            $.ajax({
                type: "POST",
                url: "https://"+osclerHost+"/bim/api/v1/dashboard/comment/"+companyId,
                ...
                success: function (data) {
                    ...
                    commentDataApiObject ={}
                        if (data.data.id>0) {
                            ...
                            html+="<b><a id='comment-"+myObject.id+"' data='"+myObject.filterData+"' about='"+myObject.pageName+"' onclick='filterReport(event);' style='cursor:pointer;font-size: 14px;background:transparent;'>"+myObject.comment+"</a></b>";
                            html+="<br/>";
  ...
  function shareDataApi(shareDataApiObject,text){
    if (text!=''){
            ...
            $.ajax({
                type: "POST",
                url: "https://"+osclerHost+"/bim/api/v1/dashboard/shareData/"+companyId,
                contentType: 'application/json',
                data: jsonData,
                cache: false,
                success: function (data) {
...
$.ajax({
        type: "GET",
        contentType: 'application/json',
        url: "https://"+osclerHost+"/company-management/api/v1/company/"+companyId+"/users?page=1&size=1000",
...
setTimeout(()=>{
 ...
 $("textarea.mention").mentionsInput({
    ...
    $(".get-mentions").click(function(tagData) {
      ...
      let mytxt;
      if(limitRange){
        mytxt = vv[0].innerText+" You have recived Top 500 Records !"
        limitRange = false;
      }else{
        mytxt = vv[0].innerText
      }
...
function getComments(pageNo){
            $.ajax({
                url: "https://"+osclerHost+"/bim/api/v1/dashboard/"+companyId+"/"+dashboardId+"/comments?page="+pageNo+"&sort=createdAt,desc",
                contentType: 'application/json',
                success:function(response){
                    ...
                    for(i=0;i<data.length;i++){
                     //   if(idMeta == "" && stateMeta == "" || data[i].postedBy.id == userId){
                         let idMetaTemp = JSON.parse(response.data[i].filterData).distributorFilter; 
                         let stateMetaTemp = JSON.parse(response.data[i].filterData).stateFilter; 
                         let idMetaStatus = idMeta == '' ? true : idMeta.includes(idMetaTemp);
                         let stateMetaStatus = stateMeta == '' ? true : stateMeta.includes(stateMetaTemp);
                       
                      if( idMetaStatus && stateMetaStatus){
                        html+='<div class="strippedDiv" style="word-wrap: break-word;padding-left:2px;">'
                        html+="<b><a id='comment-"+data[i].id+"' data='"+data[i].filterData+"' name='"+data[i].slicerData+"' about='"+data[i].pageName+"' onclick='filterReportEvent(event)' style='cursor:pointer;font-size: 14px;background:transparent;'>"+data[i].comment+"</a></b>";                        html+="<br/>";
                        html+="Chart Name: <span>"+data[i].reportName+"</span><br/>";
                        html+="Report Name: <span>"+data[i].pageDisplayName+"</span><br/>";
                        html+="Posted By <span>"+data[i].postedBy.name +"</span> at <span>"+dateTimeToCurrentTimezone(data[i].createdAtDate)+"</span>";
                        html+="<br/>";
                        html+="<br/>";
                        html+="<br/>";
                        html+='</div>';
        ...
        function toggleChatWidth(){
                ...
                if($("#chatbot-template").css("width")=="360px"){
                    ...
                    $("#chatbot-template").css("width",minWidth-100);
             ...
             function shareDataAPIPreCall() {
                    currentPageName="";
                    allowedDomainsArr = [];
                    ajaxBaseURL= this.baseURL; //$('#baseURL').val();
                    restUrl = this.restBaseURL //$('#restBaseURL').val();
                    allowedDomainsString = this.allowedEmailDomains ;//$('#allowedEmailDomains').val();
                   ...
                   // htmlString+='<i class="fa fa-eraser" style="cursor: pointer;display:none"></i>';
                    ...
                    htmlString+='<iframe allow="microphone" id="chatbot-template" style="margin-right: 0px;padding:0px;"  src="https://'+osclerHost+'/bim/api/v1/dashboardEmbed/chatbot/'+companyId+'/'+dashboardId+'"></iframe>';
                    htmlString+='<div id="chatbot-tab" style="margin-top: 330px;margin-right:-150px;position: absolute;">';    
                    htmlString+='<img onclick="javascript:toggleChatWidth();" style="cursor: pointer;width: 22px;padding-right: 8px;padding-top: 0px;"/>Ask Osler';
                    htmlString+='</div>';
                    htmlString+='<div id="comment-tab" style="margin-top: 160px;position: absolute;">Observations</div>';
                    htmlString+='</div>';
